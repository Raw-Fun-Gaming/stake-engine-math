...
def format_json_with_compact_names(data):
    ...
    Handles both compact and verbose format versions:
    - Verbose (2.0-verbose): {"name": "L5"} -> keeps on single line
    ...
    # Use regex to find and compact simple name objects (verbose format)
    ...
    pretty_json = re.sub(
        multiline_pattern, r'{"name": "\1"}', pretty_json, flags=re.MULTILINE
    )

    # Compact position objects (verbose format) to single line
    # Pattern: {"reel": 0, "row": 2} -> keep on one line
        ...
        if is_jsonl:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"  ⚠️  Standard JSON parsing failed, attempting array parsing: {e}"
                    )
                        ...
                        try:
                            ...
                            try:
                                ...
                                if last_valid_pos > 0:
                                    ...
                                    json_objects.append(parsed)
                                    print(
                                        f"  ✅ Recovered malformed JSON object by truncating extra data"
                                    )
                                else:
                                    print(
                                        f"  ⚠️  Warning: Could not recover malformed JSON object: {e}"
                                    )
                                    print(
                                        f"       Object content: {obj_content[:100]}..."
                                    )
                            except json.JSONDecodeError:
...
def main():
    ...
    if not all_files:
        print(
            f"No .json or .jsonl files with 'books' in their name found in {game_dir}"
        )
        sys.exit(0)
