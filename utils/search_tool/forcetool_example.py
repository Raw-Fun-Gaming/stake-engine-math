...
def run(game: str, mode: str):
    ...
    single_serach_condition = {"game_type": "free_game", "symbol": "H1", "kind": "5"}
    match_ids = ForceObject.find_partial_key_match(single_serach_condition, mode)
    ForceObject.print_search_results(
        single_serach_condition, match_ids, "test_single_search.json", mode
    )

    # Find the intersection of partial key matches
    multi_search_condition = [
        {"game_type": "base_game", "kind": "5", "symbol": "scatter"},
        {"game_type": "free_game", "symbol": "H1", "kind": "5"},
    ]
    intersection_ids = ForceObject.find_union_key_match(multi_search_condition, mode)
    ForceObject.print_search_results(
        multi_search_condition, intersection_ids, "test_multi_search.json", mode
    )

    # Find wins witin a specified payout range (1000x >= X > 2000x)
    payout_ids = ForceObject.find_payout_range_ids(
        method="RANGE",
        min_payout=int(1000 * 100),
        max_payout=int(2000 * 100),
        count_limit=100,
    )
    ForceObject.print_search_results(
        {"method": "payout_search"}, payout_ids, "test_range_search.json", mode
    )
